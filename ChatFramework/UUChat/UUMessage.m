...
- (NSString *)changeTheDateString:(NSString *)Str
{
    ...
    if ([lastDate hour]>=5 && [lastDate hour]<12) {
        period = NSLocalizedStringFromTable(@"AM",@"ChatFrameworkLocalized",nil);
        hour = [NSString stringWithFormat:@"%02d",(int)[lastDate hour]];
    }else if ([lastDate hour]>=12 && [lastDate hour]<=18){
        period = NSLocalizedStringFromTable(@"PM",@"ChatFrameworkLocalized",nil);
        hour = [NSString stringWithFormat:@"%02d",(int)[lastDate hour]-12];
    }else if ([lastDate hour]>18 && [lastDate hour]<=23){
        period = NSLocalizedStringFromTable(@"Night",@"ChatFrameworkLocalized",nil);
        hour = [NSString stringWithFormat:@"%02d",(int)[lastDate hour]-12];
    }else{
        period = NSLocalizedStringFromTable(@"Dawn",@"ChatFrameworkLocalized",nil);
        hour = [NSString stringWithFormat:@"%02d",(int)[lastDate hour]];
    }
